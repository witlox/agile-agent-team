...
class KanbanBoard:
    ...
    async def pull_ready_task(self) -> Optional[Dict]:
        ...
        """
        wip = await self.db.get_wip_count("in_progress")
        if wip >= self.wip_limits.get("in_progress", 4):
            return None

        ready_cards = await self.db.get_cards_by_status("ready")
        if not ready_cards:
            return None
    ...
    async def get_snapshot(self) -> Dict:
        ...
        for status in STATUSES:
            snapshot[status] = await self.db.get_cards_by_status(status)
        return snapshot
