...
class DailyStandupSession:
        ...
        for owner_id, navigator_id in active_pairs:
            ...
            if task:
                report = await self._pair_reports(
                    (owner_id, navigator_id), task, sprint_num, day_num
                )
    ...
    async def _pair_reports(
        self, pair: tuple, task: Dict, sprint_num: int, day_num: int
    ) -> StandupReport:
        ...
        report = StandupReport(
            ...
            completed_yesterday=task_progress.get(
                "completed_yesterday",
                f"Started work on {task['title']}"
                if day_num == 2
                else "Continued implementation",
            ),
            plan_today=task_progress.get(
                "plan_today",
                f"Complete {task['title']}"
                if task_progress.get("progress", 0) > 70
                else "Continue implementation and write tests",
            ),
        )
    ...
    async def _dev_lead_facilitates(
        self, reports: List[StandupReport], sprint_num: int, day_num: int
    ) -> StandupOutcome:
        ...
        for report in reports:
            for blocker in report.needs_dev_lead_decision:
                blockers_to_resolve.append(
                    {"pair": report.pair, "task": report.task_id, "blocker": blocker}
                )
        ...
        for report in reports:
            for discovery in report.architectural_discoveries:
                alignment = await self._align_architecture(
                    report.pair, discovery, sprint_num
                )
    ...
    async def _dev_lead_makes_decision(
        self, blocker: str, sprint_num: int, day_num: int
    ) -> str:
...
Your decision:"""
    ...
    async def _coordinate_dependency(
        self, blocked_pair: tuple, dependency: Dict, day_num: int
    ) -> Dict:
    ...
    async def _align_architecture(
        self, pair: tuple, discovery: str, sprint_num: int
    ) -> Dict:
...
Your guidance:"""
        ...
        return {"discovery": discovery, "decision": guidance.strip(), "impact": [pair]}
    ...
    async def _get_task_progress(self, task_id: str, day_num: int) -> Dict:
        ...
        return {
            ...
            "plan_today": "Continue implementation"
            if progress_estimate < 80
            else "Finalize and test",
        }
